#!/usr/bin/env python3
"""Generate an editable SVG of a deep RNN unrolled in time.

Layers l_1, l_2, ..., l_L (arbitrary depth, shown with a vertical ellipsis),
each recurrent (horizontal arrows) and unrolled over time steps t = 1,2,3
(superscript). Output: clean SVG with real <text>/<line>/<polygon> nodes so it
can be dropped into PowerPoint and "Convert to Shape" for full editing.
"""
...
# horizontal recurrent arrows on each hidden layer row
for rkey in ('l1', 'l2', 'lL'):
    y = ROW[rkey]
    hline(COLS[1] + PADH, COLS[2] - PADH - 2, y)
    hline(COLS[2] + PADH, COLS[3] - PADH - 2, y)
...
out = "figures/deep_rnn_unrolled.svg"
